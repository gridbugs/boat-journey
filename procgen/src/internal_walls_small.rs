...
fn find_internal_wall_candidates<A: StaticAxis>(
    ...
    let high =
        (top_left + rect.size.get_static::<A::Other>()).saturating_sub(min_distance_from_wall);
...
impl ExternalWalls {
    ...
    fn internal_wall_candidates(&self, rect: &Rect, min_rect_size: Size) -> InternalWallCandidates {
        ...
        let x_coord_of_vertical_wall_candidates =
            find_internal_wall_candidates(&self.vertical, rect, min_rect_size);
        let x_coord_of_vertical_wall_candidates = InternalWallCandidatesInAxis {
            candidates: Vec::new(),
...
fn split_rect_with_wall<R: Rng, A: StaticAxis>(
    ...
    let right = Rect {
        coord: rect.coord
            + left.size.to_coord().unwrap().set_static::<A>(0)
            + Coord::new_static_axis::<A>(0, 1),
